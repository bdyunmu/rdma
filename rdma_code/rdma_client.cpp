...
RDMAClient::RDMAClient()
{
    ...
    err = pthread_create(&ntid, NULL, thr_fn, (void *)this);
    if (err != 0)
    {
...
static void* thr_fn(void* arg)
{
  RDMAClient *ptr = (RDMAClient *)arg;

  pthread_mutex_lock(&ptr->mutex);

  while(ptr->thr_fin == false)
  {
    // there are some requests  todo
    ptr->requests.size() > 0;
    Event et = ptr->requests.front();
    ptr->requests.pop();
    // do the request
    if (et.type == EVENT_READ)
    {
      ptr->read(et.localOffset, et.remoteOffset, et.size);
    }
    else if (et.type == EVENT_WRITE)
    {
      ptr->write(et.localOffset, et.remoteOffset, et.size);
    }
    else
    {
